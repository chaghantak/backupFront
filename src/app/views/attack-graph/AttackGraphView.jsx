...
import {addChain} from "../../redux/actions/ChainAction"
import {getGraphs, getGraph, updateGraph, splitGraph} from "../../redux/actions/GraphAction"
...
class AttackGraphView extends Component{
  constructor(props){
    super(props);
    this.network = {};
  ...
  deleteObject = async (data, callback) => {
    callback(data);
    await this.setState({"prevGraph" : this.state.graph});
    await this.setState({ graph : {...this.state.graph, nodes : this.network.body.data.nodes.get(), edges : this.network.body.data.edges.get()}})
  ...
  handleNodeDialogClose = async (caller) => {
    if(caller=="confirm"){
      ...
      await this.setState({"prevGraph" : this.state.graph});
      await this.setState({ graph : {...this.state.graph, nodes : this.network.body.data.nodes.get()}})
      this.props.updateGraph(this.state.graph);
    }
  ...
  handleEdgeDialogClose = async (caller) => {
    if(caller=="confirm"){
      ...
      this.callback(this.data);
      await this.setState({"prevGraph" : this.state.graph});
      await this.setState({ graph : {...this.state.graph, edges : this.network.body.data.edges.get()}})
      this.props.updateGraph(this.state.graph);
    }
  ...
  copyAsJson = async () => {
    await this.props.getGraph(this.props.match.params.id, false);
    if(!navigator.clipboard) {
      var textArea = document.createElement("textarea");
      textArea.value = JSON.stringify(this.props.graph.graph);
      ...
      navigator.clipboard.writeText(JSON.stringify(this.props.graph.graph)).then(() => {
        this.props.showMsg("Copying text command was " + msg, "success");
      }, () => {
        this.props.showMsg("Unable to copy", "error");
      })
  ...
  splitGraph = async () => {
    await this.props.splitGraph(this.state.graph)
    for(var i=0;i<this.props.graph.splited_chains.length;i++){
      await this.props.addChain(this.props.graph.splited_chains[i]);
    }
  }
  ...
  selectEvidence = () => {
    ...
    this.network.body.data.nodes.map((node) => {
      if(selection.nodes.includes(node.id)){
        node.evidence = !node.evidence ? true : false;
        node.font = node.evidence ? { background : "yellow" } : { background : "none" };
  ...
  revertGraph = async () => {
    await this.setState({"graph" : this.state.prevGraph});
    this.network.body.data.nodes.clear();
    this.network.body.data.edges.clear();
    this.network.body.data.nodes.add(this.state.graph.nodes);
    this.network.body.data.edges.add(this.state.graph.edges);
    this.props.updateGraph(this.state.graph);
  }
  ...
  async componentDidMount() {
    await this.props.getGraph(this.props.match.params.id, true);
    this.setState({"graph" : this.props.graph.graph});
    this.setState({"prevGraph" : this.props.graph.graph});
    this.network = new Network(this.appRef.current, {nodes : this.state.graph.nodes, edges : this.state.graph.edges}, this.options);
    ...
    return (
      <div className="m-sm-30">
        <div  className="mb-sm-30">
          <Breadcrumb
            routeSegments={[
              ...
              { name: this.props.graph.graph.title, path: "/attack-graph/"+this.props.graph.graph.id }
            ]}
          />
        </div>
        <Card className="px-6 pt-2 pb-4" >
          ...
          <Button variant="contained" className={classes.button} onClick={() => {this.revertGraph();}}>
            Revert
          </Button>
          <div style={{float : "right"}}>
            <Button variant="contained" color="primary" className={classes.button} onClick={() => {this.calculatePosterior();}}>
              Calculate posterior
            </Button>
            <Button variant="contained" color="secondary" className={classes.button} onClick={() => {this.selectEvidence();}}>
              Select evidences
            </Button>
            <Button variant="contained" className={classes.button} onClick={() => {this.splitGraph();}}>
              Split graph to chains
            </Button>
...
export default withStyles(styles)(connect(mapStateToProps, mapDispatchToProps)(AttackGraphView));
